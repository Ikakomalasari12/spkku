...
def save_smartphone_data(nama_smartphone, kapasitas_baterai, performa, kualitas_kamera, harga, rating_pengguna):
    cnx = mysql.connector.connect(user='root', password='', host='localhost', database='spk')
    cursor = cnx.cursor()
    query = ("INSERT INTO smartphone "
             "(nama_smartphone, kapasitas_baterai, performa, kualitas_kamera, harga, rating_pengguna) "
             "VALUES (%s, %s, %s, %s, %s, %s)")
    cursor.execute(query, (nama_smartphone, kapasitas_baterai, performa, kualitas_kamera, harga, rating_pengguna))
    cnx.commit()
    cursor.close()
    cnx.close()
...
def get_smartphone_data():
    cnx = mysql.connector.connect(user='root', password='', host='localhost', database='spk')
    cursor = cnx.cursor()
    query = "SELECT nama_smartphone, kapasitas_baterai, performa, kualitas_kamera, harga, rating_pengguna FROM smartphone"
...
def update_data(nama_smartphone, kapasitas_baterai, performa, kualitas_kamera, harga, rating_pengguna):
    cnx = mysql.connector.connect(user='root', password='', host='localhost', database='spk')
    cursor = cnx.cursor()
    query = ("UPDATE smartphone SET kapasitas_baterai = %s, performa = %s, kualitas_kamera = %s, "
             "harga = %s, rating_pengguna = %s WHERE nama_smartphone = %s")
    cursor.execute(query, (kapasitas_baterai, performa, kualitas_kamera, harga, rating_pengguna, nama_smartphone))
    cnx.commit()
    cursor.close()
    cnx.close()
