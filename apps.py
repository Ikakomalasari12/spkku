...
ranking_dict = {
    ...
    'performa': {'Rendah': 1 , 'Menengah': 2, 'Tinggi': 3, 'Sangat Tinggi': 4},
    'kualitas_kamera': {'Rendah': 1, 'Menengah': 2, 'Tinggi': 3, 'Sangat Tinggi': 4},
...
with st.sidebar:
    menu = option_menu(
        menu_title = "Menu",
        options = ["Overview", "Create", "Read", "Update", "Delete", "Ranking"],
...
if menu == 'Overview':
    ...
    performa = st.radio('Performa', ['Rendah', 'Menengah', 'Tinggi', 'Sangat Tinggi'])
    kualitas_kamera = st.radio('Kualitas Kamera', ['Rendah', 'Menengah', 'Tinggi', 'Sangat Tinggi'])
    ...
    if st.button('Submit'):
        ...
        performa_rank = convert_to_ranking(performa, ranking_dict['performa'])
        ...
        save_smartphone_data(nama_smartphone, kapasitas_baterai_rank, performa_rank, kualitas_kamera_rank, harga_rank, rating_pengguna_rank)
    ...
    try:
        ...
        df = pd.DataFrame(data, columns=['nama_smartphone', 'kapasitas_baterai', 'performa', 'kualitas_kamera', 'harga', 'rating_pengguna'])
    ...
    if all_data and len(all_data[0]) == 7:
        ...
        df = pd.DataFrame(all_data, columns=['id', 'nama_smartphone', 'kapasitas_baterai', 'performa', 'kualitas_kamera', 'harga', 'rating_pengguna'])
        ...
        numeric_columns = ['kapasitas_baterai', 'performa', 'kualitas_kamera', 'harga', 'rating_pengguna']
        ...
        benefit_criteria = ['kapasitas_baterai', 'performa', 'kualitas_kamera', 'rating_pengguna']  # Kriteria yang ingin dimaksimalkan
        ...
        st.write(weighted_df[['nama_smartphone', 'kapasitas_baterai', 'performa', 'kualitas_kamera', 'harga', 'rating_pengguna', 'D+', 'D-', 'V']])
